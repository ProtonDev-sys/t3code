...
function formatProviderUsageSummary(provider: ProviderRuntimeEvent["provider"]): string {
  const label =
  ...
  return `${label} usage updated`;
}

function buildContextWindowActivityPayload(event: ProviderRuntimeEvent):
...
function withThreadModelSelection(
  payload: ReturnType<typeof buildContextWindowActivityPayload>,
  thread: { readonly modelSelection?: { readonly model?: string } | null },
) {
  if (!payload) {
    return undefined;
  }
  ...
  return {
    ...payload,
    ...(model ? { model, modelSelection: thread.modelSelection } : {}),
  };
}
...
function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "thread.token-usage.updated": {
      const payload = withThreadModelSelection(buildContextWindowActivityPayload(event), thread);
      if (!payload) {
        return [];
      }
      ...
      return [
        {
          ...
          summary: formatProviderUsageSummary(event.provider),
          payload: {
            provider: event.provider,
            ...(event.providerInstanceId !== undefined
              ? { providerInstanceId: event.providerInstanceId }
...
const make = Effect.gen(function* () {
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      const shouldApplyThreadLifecycle = (() => {
        ...
        switch (event.type) {
          ...
          case "turn.completed":
            ...
            // Some runtimes can only emit a thread-scoped completion. If it is
            // scoped to this thread and does not conflict with another turn id,
            // use it to clear the active lifecycle instead of leaving the
            // thread permanently running.
      ...
      const activities = runtimeEventToActivities(event, thread);
