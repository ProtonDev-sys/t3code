...
function OpenCommandPaletteDialog() {
  ...
  const promoteSidebarThread = useCommandPaletteStore((store) => store.promoteSidebarThread);
  ...
  const openProjectFromSearch = useMemo(
    () => async (project: (typeof projects)[number]) => {
      ...
      if (latestThread) {
        promoteSidebarThread(
          scopedThreadKey(scopeThreadRef(latestThread.environmentId, latestThread.id)),
        );
        await navigate({
    ...
    () =>
      buildThreadActionItems({
        ...
        runThread: async (thread) => {
          promoteSidebarThread(scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)));
          await navigate({
            to: "/$environmentId/$threadId",
  ...
  const addProjectEnvironmentItems = useMemo<CommandPaletteActionItem[]>(
    () =>
  ...
  const filteredGroups = useMemo(
    ...
    [activeGroups, allThreadItems, currentView, deferredQuery, projectSearchItems],
  );
  ...
  const threadPrewarm = useMemo(() => {
    const refs = getCommandPaletteThreadPrewarmRefs({ groups: displayedGroups });
    return {
      refs,
      key: refs.map((ref) => `${ref.environmentId}:${ref.threadId}`).join("|"),
    };
  }, [displayedGroups]);

  useEffect(() => {
    return prewarmThreadDetailSubscriptions(threadPrewarm.refs, {
      initialDelayMs: 200,
      intervalMs: 120,
      retainMs: 5_000,
    });
